...
data class ErrorResponse(
    ...
    companion object {
        ...
        fun of(ex: MissingKotlinParameterException) = ErrorResponse(
                exception = ex.javaClass.simpleName,
                type = Type.VALIDATION,
                errors = listOf(FieldError(ex.parameter.name
                        ?: "", listOf("Field is required")))
        )

        fun of(ex: Throwable) = ErrorResponse(
                exception = ex.javaClass.simpleName,
                type = Type.GENERAL,
                errors = listOf(ValidationErrorMessage(listOf(ex.localizedMessage)))
        )
        ...
        fun of(ex: MismatchedInputException) = ErrorResponse(
                ...
                errors = ex.path
                        ...
                        .map { FieldError(it.fieldName, listOf("Field has a wrong type")) }
        )
